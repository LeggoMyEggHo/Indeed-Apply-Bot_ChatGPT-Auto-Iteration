...
HandleOpenChatGPT(iterationCount, maxIterations) {
    ...
    if !WinWaitActive("chatgpt.com - Google Chrome",, 3) {
        if !WinWaitActive("ChatGPT - Google Chrome",, 3) {
        ...
        try {
            verifyYouAreHumanElement := rootElement.WaitElement({ LocalizedType: "check box", Name: "Verify you are human" }, 5000)
            ;MsgBox("Found Verify you are human checkbox")
        } catch {
            ;MsgBox("Failed to find Verify you are human checkbox")
            verifyYouAreHumanElement := {}
        }

        if verifyYouAreHumanElement != {} {
            ;MsgBox("Clicking Verify you are human checkbox")
            ClickElementByPath(verifyYouAreHumanElement, rootElement)
        } else {
            ;MsgBox("No Verify you are human checkbox to click")
        }
    }
