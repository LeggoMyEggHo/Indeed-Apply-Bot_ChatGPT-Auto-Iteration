...
global checkboxPatterns := Map("ignoreNA", "N/A", "ignoreNone", "None")
...
LoadConfig() {
    ...
    global commonResponses, configFile, ExcludeList, ExcludeJobTypes, jobSearchKeywords
...
SetupConfig(section := "", key := "", value := "") {
    ...
    global jobSearchKeywordsEdit := jobSearchKeywords
...
SaveConfig() {
    ...
    global jobSearchKeywords := jobSearchKeywordsEdit.Value
...
HandleKeywords(associatedLabel) {
    ...
    if InStr(associatedLabel, "solutions") && InStr(associatedLabel, "have you sold") {
        ...
        return email . " " . phone
    } else if InStr(associatedLabel, " phone ") && !InStr(associatedLabel, " email ") && !InStr(associatedLabel, "sales") {
...
HandleCheckboxes(rootElement) {
    global resume, checkboxPatterns, strCheckboxPatterns, checkBoxSuccess := false, strCheckboxes := "", checkBoxSuccess := false, associatedLabel
    global answerReview, associatedLabel
    ;try {
        ...
        for checkbox in checkboxes {
            ...
            if toggleState := togglePattern.ToggleState == 1 {
                ToolTip("Checkbox " checkbox.Name " for " parentElement.Name " is already selected. Skipping.")
                continue
            }
            ...
            ;MsgBox("Checkbox for " checkboxText " is not on the ignore list. Clicking checkbox.")
            ClickElementByPath(checkbox, rootElement)
        }
        childrenCheckboxes := parentElement.FindAll({ LocalizedType: "check box" })
        if !checkBoxSuccess && answerReview == true {
            ToolTip("No answer found. Asking ChatGPT.")  ; Debugging output

            answer := ChatGPT("Question: " parentElement.Name " Answer Options: " strCheckboxes, "Using the following resume for context, directly answer the question using one of the options provided. Do not provide any other output. Example Options: Yes or No Example Answer: Yes", resume, , , , getGPTFeedback := "True")
            if answerReview {
                inputBoxValue := InputBox("Answer review: " strCheckboxes "`nfor Label: " associatedLabel,,, inputValue)
                inputValue := inputBoxValue.Value
                    ...
                    if InStr(strCheckboxes, i) {
                        for childCheckBox in childrenCheckboxes {
                            if childCheckBox.Name == i {
                                ...
                                FileAppend("Found i: " i " and checked checkbox: " checkbox.Name " for parentElement.Name: " parentElement.Name "`n", "all_questions.txt")
                            }
                        }
                    } else {
                ...
                for i in answer {
                    if InStr(strCheckboxes, i) {
                        ...
                        ToolTip("Failed to grab a proper answer from ChatGPT.")  ; Debugging output
                        FileAppend("Failed to grab a proper answer from ChatGPT.`n `nQuestion: " parentElement.Name "`nAnswer Options:`n" strCheckboxes "`nChatGPT response: " answer "`n`n", logFile)
                    }
                }
            }
        }
