...
; Function within HandleJobSearch.ahk to search for jobs using jobSearchKeywords and sets curreParentElement to 0. May need CloseTabWithCheck() before calling
SearchForJobs() {
    Sleep(300)
...
; Function to handle the job search page within HandleJobSearch.ahk
    ...
    Loop {
        ...
        while (currentParentElement < parentLength) {
            ...
            if !CheckIfValid(rootElement, parentPath) {
                ...
                if applyNowElement == "" {
                    try {
                        scheduleAnInterviewElement := rootElement.FindFirst({ LocalizedType: "button", Name: "Schedule an interview opens in a new tab" })
                        ClickElementByPath(scheduleAnInterviewElement, rootElement, , 2000)
                    } catch {
                        scheduleAnInterviewElement := ""
                    }

                    if scheduleAnInterviewElement == "" {
                        try {
                            indeedApplyButtonElement := rootElement.FindFirst({ LocalizedType: "button", AutomationID: "indeedApplyButton" })
                            ClickElementByPath(indeedApplyButtonElement, rootElement, , 2000)
                        } catch {
                            indeedApplyButtonElement := ""
                            failedContinue := true
                        }
                    }
                }
                ...
                if failedContinue {
                    jobDetails := parentElement.FindAll()
                    companiesStr := ""
                    for jobDetail in jobDetails {
                        companiesStr .= jobDetail.Name "`n"
                    }
                    FileAppend("Failed to continue after checking for valid job details.`n" parentPath "`n" companiesStr "`n", "failedContinue_log.txt")
...
; Function within HandleJobSearch.ahk that checks if the parent element is valid.
CheckIfValid(rootElement, parentPath) {
    success := false
    global excludeList, excludeJobTypes ; Grab the global jobSearchKeywords, excludeList, and excludeJobTypes variables from #Include JobApplicationQA.ahk
    global cleanedJobName
    ; Check for the parent element
    parentElement := rootElement.WaitElementFromPath(parentPath, 2000)  ; 2-second timeout
    if !parentElement {
...
; Function to check if the job is valid. Lives within HandleJobSearch.ahk
CheckIfJobValid(rootElement, parentPath) {
    global cleanedJobName, notInterested := false
...
; Function within HandleJobSearch.ahk to handle clicking the "Next Page" link with randomization
HandleNextPage(rootElement) {
